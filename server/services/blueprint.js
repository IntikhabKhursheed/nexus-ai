...
const blueprints = {
  ...
  default: [
    ...
    // Generate tasks based on extracted concepts
    function(concepts, goal) {
      if (concepts.length === 0) {
        concepts.push('core functionality');
      }

      const generatedTasks = concepts.slice(0, 7).map((concept, index) => {
        const priority = index < 2 ? 'High' : index < 4 ? 'Medium' : 'Low';
        return {
          title: `Build ${concept} for ${goal}`,
          description: `Implement ${concept} functionality as part of ${goal} project`,
          priority: priority
        };
      });
...
function getAnchorTasks(projectType) {
  return blueprints[projectType] || blueprints.default;
}

module.exports = { detectProjectType, getAnchorTasks };
